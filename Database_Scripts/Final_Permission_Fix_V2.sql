-- =====================================================
-- ANSMS Pro - Final Permission System Fix V2
-- =====================================================
-- This script fixes the permission system with ValueKey support
...
-- Check if Roles category exists in ConfigurationCategories, if not create it
...
-- Ensure basic roles exist in ConfigurationValues
...
BEGIN
    ...
    VALUES (@RolesCategoryId, 'ADMIN', 'Admin', 1)
    PRINT 'Admin role created'
END

IF NOT EXISTS (SELECT 1 FROM ConfigurationValues cv 
               INNER JOIN ConfigurationCategories cc ON cv.CategoryId = cc.CategoryId 
               WHERE cc.CategoryName = 'Roles' AND cv.ValueText = 'Supervisor')
BEGIN
    DECLARE @RolesCategoryId2 int
    SELECT @RolesCategoryId2 = CategoryId FROM ConfigurationCategories WHERE CategoryName = 'Roles'
    
    INSERT INTO ConfigurationValues (CategoryId, ValueKey, ValueText, IsActive) 
    VALUES (@RolesCategoryId2, 'SUPERVISOR', 'Supervisor', 1)
    PRINT 'Supervisor role created'
END

IF NOT EXISTS (SELECT 1 FROM ConfigurationValues cv 
               INNER JOIN ConfigurationCategories cc ON cv.CategoryId = cc.CategoryId 
               WHERE cc.CategoryName = 'Roles' AND cv.ValueText = 'Staff')
BEGIN
    DECLARE @RolesCategoryId3 int
    SELECT @RolesCategoryId3 = CategoryId FROM ConfigurationCategories WHERE CategoryName = 'Roles'
    
    INSERT INTO ConfigurationValues (CategoryId, ValueKey, ValueText, IsActive) 
    VALUES (@RolesCategoryId3, 'STAFF', 'Staff', 1)
...
IF NOT EXISTS (SELECT 1 FROM ConfigurationValues cv 
               INNER JOIN ConfigurationCategories cc ON cv.CategoryId = cc.CategoryId 
               WHERE cc.CategoryName = 'Divisions' AND cv.ValueText = 'AFTN')
BEGIN
    ...
    INSERT INTO ConfigurationValues (CategoryId, ValueKey, ValueText, IsActive) 
    VALUES (@DivisionsCategoryId, 'AFTN', 'AFTN', 1)
    PRINT 'AFTN division created'
END

IF NOT EXISTS (SELECT 1 FROM ConfigurationValues cv 
               INNER JOIN ConfigurationCategories cc ON cv.CategoryId = cc.CategoryId 
               WHERE cc.CategoryName = 'Divisions' AND cv.ValueText = 'AIS')
BEGIN
    DECLARE @DivisionsCategoryId2 int
    SELECT @DivisionsCategoryId2 = CategoryId FROM ConfigurationCategories WHERE CategoryName = 'Divisions'
    
    INSERT INTO ConfigurationValues (CategoryId, ValueKey, ValueText, IsActive) 
    VALUES (@DivisionsCategoryId2, 'AIS', 'AIS', 1)
    PRINT 'AIS division created'
END

IF NOT EXISTS (SELECT 1 FROM ConfigurationValues cv 
               INNER JOIN ConfigurationCategories cc ON cv.CategoryId = cc.CategoryId 
               WHERE cc.CategoryName = 'Divisions' AND cv.ValueText = 'CNS')
BEGIN
    DECLARE @DivisionsCategoryId3 int
    SELECT @DivisionsCategoryId3 = CategoryId FROM ConfigurationCategories WHERE CategoryName = 'Divisions'
    
    INSERT INTO ConfigurationValues (CategoryId, ValueKey, ValueText, IsActive) 
    VALUES (@DivisionsCategoryId3, 'CNS', 'CNS', 1)
...
-- Get Role IDs using proper JOIN
...
SELECT @SupervisorRoleId = cv.ValueId 
FROM ConfigurationValues cv 
INNER JOIN ConfigurationCategories cc ON cv.CategoryId = cc.CategoryId 
WHERE cc.CategoryName = 'Roles' AND cv.ValueText = 'Supervisor'

SELECT @StaffRoleId = cv.ValueId 
FROM ConfigurationValues cv 
INNER JOIN ConfigurationCategories cc ON cv.CategoryId = cc.CategoryId 
WHERE cc.CategoryName = 'Roles' AND cv.ValueText = 'Staff'

-- Insert role permissions only if roles exist
IF @AdminRoleId IS NOT NULL
BEGIN
    INSERT INTO RolePermissions (RoleId, PermissionId)
    SELECT @AdminRoleId, PermissionId FROM Permissions WHERE IsActive = 1
    PRINT 'Admin role permissions inserted'
END
...
BEGIN
    ...
    PRINT 'Supervisor role permissions inserted'
END
...
BEGIN
    ...
    PRINT 'Staff role permissions inserted'
END
